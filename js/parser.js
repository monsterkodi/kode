...
Parser = (function ()
{
    ...
    Parser.prototype["use"] = function (obj, tokens)
    {
        ...
        toUse = []
        token = tokens.shift()
        while (token && token.type !== 'nl')
        {
            module = token.text
            while (tokens.length && tokens[0].col === token.col + token.text.length)
            {
                token = tokens.shift()
                module += token.text
            }
    ...
    Parser.prototype["curly"] = function (open, tokens)
    {
        ...
        if (!close)
        {
            _k_.dbg(".", 786, 12, "exps", exps)
            _k_.dbg(".", 787, 12, "tokens", tokens)
        }
        this.pop('{')
