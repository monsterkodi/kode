    ...
    exps: (rule, tokens, stop) ->
        ...
        while tokens.length
            ...
            b = switch @stack[-1]
                ...
                when rule                   then tokens[0].text == stop and tokens[0].type != 'var'
                else false

            if b then @verb "exps break for #{tokens[0].text} and stack top" @stack ; break 
                
            if stop and tokens[0].text == stop and tokens[0].type != 'var'
                @verb "exps break for #{tokens[0].text} and stop" stop 
                break 
